...
LAST_BACKUP_TIME=$(find $GLB_SNAPSHOT -type f -name '*.sngz' -exec sh -c "strings {} | awk 'NR==2{print}'" \; | sort -n | head -1 | xargs -I {} date +'%Y-%m-%d %H:%M:%S' -d @{})
...
archiveLFS() {
  ...
  # echo start time: `date`
  time tar cgP $GLB_SNAPSHOT/snapshot-LFS.sngz $GLB_LFS | pigz -1 $GLB_WEEKLY/$BACKUP_VERSION/$TIMESTAMP-LFS.tar.gz
  # echo end time: `date`
}
...
archiveByTime() {
  ...
  then
    ...
    time tar cgP $GLB_SNAPSHOT/$REPO_NAME.sngz $line | pigz -1 $GLB_DAILY/$BACKUP_VERSION/$TIMESTAMP-$REPO_NAME.tar.gz
