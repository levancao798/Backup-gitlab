...
#       $ sudo chmod g+rw /u01/os_vtt_gitlab/gitlab/ -R
...
# Crontab:
#      $ crontab -e
#      $ "0 0 * * SAT bash /u01/os_vtt_gitlab/gitlab/shell-script/backup-gitlab-weekly.sh" #At 00:00 on Saturday.
...
archiveLFS() {
  ...
  # echo start time: `date`
  time tar cgP $GLB_SNAPSHOT/LFS.sngz $GLB_LFS | pigz -1 $GLB_WEEKLY/$BACKUP_VERSION/$TIMESTAMP-LFS.tar.gz
  # echo end time: `date`
}
...
archiveRepo() {
  ...
  then
    ...
    time tar cgP $GLB_SNAPSHOT/$REPO_NAME.sngz $line | pigz -1 $GLB_WEEKLY/$BACKUP_VERSION/$TIMESTAMP-$REPO_NAME.tar.gz
